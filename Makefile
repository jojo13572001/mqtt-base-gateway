...
LDFLAGS += -L json-c-0.11/.libs -ljson-c -L/sourcecode/ar/MT7620AR_rick/RT288x_SDK/source/oring_ar/library/open_source/openssl-0.9.8l
jsondir = -I json-c-0.11
...
SAMPLE_FILES_C = paho_cs_pub paho_cs_sub MQTTClient_publish MQTTClient_publish_async MQTTClient_subscribe Gateway
...
CCFLAGS_SO = -g -fPIC $(CFLAGS) -Os -Wall -I$(blddir_work)
FLAGS_EXE = $(LDFLAGS) -I $(srcdir) -lpthread -L ${blddir} ${jsondir}
FLAGS_EXES = $(LDFLAGS) -I $(srcdir) ${START_GROUP} -lpthread -lssl -lcrypto ${END_GROUP} -L ${blddir} ${jsondir}
...
${SYNC_SAMPLES}: ${blddir}/samples/%: ${srcdir}/samples/%.c $(MQTTLIB_CS_TARGET)
	${CC} ${srcdir}/queue.c ${srcdir}/queue_internal.c ${srcdir}/samples/Json_parser.h -o $@ $< -l${MQTTLIB_CS} ${FLAGS_EXES}
